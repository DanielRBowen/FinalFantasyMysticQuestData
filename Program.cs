...
static void UpdateBestiaryFromMonsters()
{
 ...
 "Dark King (1st Form)","Dark King (2nd Form)","Dark King (3rd Form)","Dark King (4th Form)","Pazuzu","Skullrus Rex"
    ...
    // Build a map of Name -> WikiUrl to scrape levels
    var nameToWiki = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);

    // First pass to repair AbilitiesJson spill per-row and collect wiki urls
    var parsedRows = new List<List<string>>();
    for (int i = 1; i < lines.Length; i++)
    {
        var row = CsvUtil.ParseCsvRow(lines[i]);
        if (row.Count == 0) { parsedRows.Add(row); continue; }
        ...
        if (!string.IsNullOrWhiteSpace(name) && !string.IsNullOrWhiteSpace(wiki))
        {
            if (!nameToWiki.ContainsKey(name)) nameToWiki[name] = wiki;
        }

        parsedRows.Add(row);
    }

    // Scrape level per monster name
    var levelByName = ScrapeLevels(nameToWiki);
    ...
    foreach (var row in parsedRows)
    {
        ...
        string level = levelByName.TryGetValue(name, out var lvl) && !string.IsNullOrWhiteSpace(lvl) ? lvl : "1";
        ...
        // Markdown table row with updated column order (includes Level; Strength instead of Attack)
    ...
    int sepIdx = headerIdx + 1; // assumes separator line next
...
// Scrape all levels from wiki URLs directly (no local cache).
static Dictionary<string, string> ScrapeLevels(Dictionary<string, string> nameToWiki)
{
    var result = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
    if (nameToWiki.Count == 0) return result;

    using var http = new HttpClient(new HttpClientHandler { AllowAutoRedirect = true });
    http.Timeout = TimeSpan.FromSeconds(10);
    http.DefaultRequestHeaders.UserAgent.ParseAdd("FinalFantasyMysticQuestData/1.0");
    http.DefaultRequestHeaders.Accept.ParseAdd("text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8");

    foreach (var kvp in nameToWiki)
    {
        string name = kvp.Key;
        ...
        try
        {
            ...
            string lvl = ParseLevelFromHtml(html);
            if (!string.IsNullOrWhiteSpace(lvl)) result[name] = lvl.Trim();
        }
        catch
        {
    ...
    static string ParseLevelFromHtml(string html)
    {
        if (string.IsNullOrWhiteSpace(html)) return string.Empty;
        var doc = new HtmlDocument();
        doc.LoadHtml(html);

        // Try common Fandom infobox structures for Level
        var xpaths = new[]
        {
 // generic pi-data with value
 "//*[@data-source='level']//*[contains(@class,'pi-data-value') or contains(@class,'pi-smart-data-value')][1]",
 // smart data value directly
 "//div[contains(@class,'pi-smart-data-value') and @data-source='level']",
 // pi-data-value with data-source on same node
 "//div[contains(@class,'pi-data-value') and @data-source='level']",
 // within smart group by matching label then value in that group
 "//h3[contains(@class,'pi-smart-data-label') and @data-source='level']/ancestor::section[contains(@class,'pi-smart-group')][1]//div[contains(@class,'pi-smart-data-value') and not(contains(@class,'label'))]"
 };
 ...
 var num = ExtractLeadingInt(txt);
 if (!string.IsNullOrWhiteSpace(num)) return num;
 }
 }

 // Last resort: any element text like "Level:12"
 var cand = doc.DocumentNode.SelectNodes("//*[contains(translate(normalize-space(.), 'LEVEL', 'level'), 'level')]");
 if (cand != null)
 {
 foreach (var c in cand)
 {
 string txt = c.InnerText ?? string.Empty;
 var m = Regex.Match(txt, @"(?i)level\s*:?\s*(\d+)");
 ...
 static string ExtractLeadingInt(string s)
 {
